...
from nnutils import mask_image_from_size


# This local implementation is no longer needed - using the one from nnutils
# def mask_image_from_size(batch_input, batch_sizes, mask_value):
#     """
#     Masks batch of images based on individual sizes.
#     
#     Args:
#         batch_input: Input tensor of shape (N, C, H, W)
#         batch_sizes: Tensor of shape (N, 2) containing height and width for each batch item
#         mask_value: Value to use for masked areas
#     
#     Returns:
#         A tensor of the same shape as batch_input with areas outside each image size
#         masked with mask_value
#     """
#     # Create a copy of the input to modify
#     N, C, H, W = batch_input.shape
#     output = batch_input.clone()
#     
#     # For each batch item
#     for n in range(N):
